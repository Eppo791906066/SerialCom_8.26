...
void HalKeyInit( void )
{
  ...
  #else
      P2SEL &= ~0x1;         /* Set P2.0 pin function to GPIO */
      P2DIR &= ~0x1;         /* Set P2.0 pin direction to Input */
  #endif
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
    ...
    HAL_KEY_SW_2_PXIFG = ~(HAL_KEY_SW_2_BIT); /* Clear any pending interrupt */
        ...
        /* Rising/Falling edge configuratinn */
        PICTL |= 0x8;                    /* set falling edge for Port2 */

        P2IEN |= 0x1;                    /* enable interrupt for P2.0 */
        P2IFG = 0x0;                     /* clear P2.0 pending flag */
...
void halProcessKeyInterrupt (void)
{
  ...
  #else
    if (P2IFG & 0x1)    /* P2.0 interrupt flag bas been set */
    {
        P2IFG = ~0x1;
        valid = TRUE;
    }
